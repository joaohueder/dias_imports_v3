  ...
  try {
    ...
    const workers = workerRows.map((w) => {
      ...
      return {
        ...
        schedule_enabled: w.schedule_enabled !== undefined ? Boolean(w.schedule_enabled) : true,
        schedule_interval_seconds: Number(w.schedule_interval_seconds) || (Number(w.schedule_interval_minutes) ? Number(w.schedule_interval_minutes) * 60 : (w.id === "w-health-01" ? 15 : 3600)),
        schedule_interval_minutes: Number(w.schedule_interval_minutes) || (Number(w.schedule_interval_seconds) ? Math.round(Number(w.schedule_interval_seconds) / 60) : (w.id === "w-reports-01" ? 5 : (w.id === "w-cron-01" ? 60 : 1))),
        status: w.status,
