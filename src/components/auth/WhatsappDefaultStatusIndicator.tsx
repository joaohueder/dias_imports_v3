...
export function WhatsappDefaultStatusIndicator() {
  ...
  const checkStatus = useCallback(async () => {
    try {
      const response = await fetch("/api/sa/settings/default-instance", {
        cache: "no-store",
      });
  ...
  return (
    <>
      ...
      >
        ...
        />
        <span className={status === "connected" ? "text-emerald-300/90" : "text-slate-400"}>WhatsApp:</span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="w-full max-w-md rounded-2xl bg-[#0b1120] border border-slate-800 p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-150">
